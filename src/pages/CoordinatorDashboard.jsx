...
import {
  ...
  ChevronRight, Star, Target, FileText, CheckCircle2, Clock
} from 'lucide-react'
...
export default function CoordinatorDashboard() {
  ...
  const navItems = [
    ...
    { label: 'Students', icon: <Users size={18} />, badge: students.length, active: activeTab === 'students', onClick: () => setActiveTab('students') },
    { label: 'Employers', icon: <Briefcase size={18} />, badge: employers.length, active: activeTab === 'employers', onClick: () => setActiveTab('employers') },
    { label: 'Placements', icon: <Trophy size={18} />, badge: applications.filter(a => a.status === 'offer').length, active: activeTab === 'placements', onClick: () => setActiveTab('placements') },
    { label: 'Documents', icon: <FileText size={18} />, active: activeTab === 'documents', onClick: () => setActiveTab('documents') },
  ]
  ...
  return (
    <DashboardShell
      ...
      navItems={navItems}
      profile={{
        full_name: profile?.full_name || 'Coordinator',
        role_label: 'University Coordinator',
      }}
      onLogout={handleLogout}
      logoMark={<LayoutGrid size={18} color="#fff" />}
    >
      <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto animate-fade-up space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          {metrics.map((m, i) => (
            <div key={i} className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-subtle" style={{ borderTop: `3px solid ${m.color}` }}>
              ...
              <div className="text-[11px] text-slate-500 font-semibold mt-1 leading-tight">{m.label}</div>
            </div>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-subtle">
              <div className="flex items-center justify-between gap-3 mb-4">
                ...
                <button onClick={() => setActiveTab('placements')} className="text-xs font-bold text-brand-600 hover:text-brand-700 inline-flex items-center gap-1">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-subtle">
              <div className="flex items-center justify-between gap-3 mb-4">
                ...
                <button onClick={() => setActiveTab('students')} className="text-xs font-bold text-brand-600 hover:text-brand-700 inline-flex items-center gap-1">
                  View all <ChevronRight size={13} />
                </button>
              </div>

              {students.slice(0, 5).length === 0 ? (
                <div className="py-8 text-center text-xs text-slate-500">No students yet</div>
              ) : (
        ...
        {activeTab === 'students' && (
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-subtle">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={exportToCSV} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50">
        ...
        {/* Employers — NOW WITH VERIFIED STATUS */}
        {activeTab === 'employers' && (
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-subtle">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-slate-900">All Employers ({employers.length})</h3>
              <div className="flex items-center gap-2 text-[11px] font-bold text-slate-500">
                <span className="inline-flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                  {employers.filter(e => e.verified).length} verified
                </span>
                <span className="text-slate-300">·</span>
                <span className="inline-flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                  {employers.filter(e => !e.verified).length} pending
                </span>
              </div>
            </div>
            ...
            {employers.length === 0 ? (
              ...
              <div className="space-y-3">
                {employers.map(e => {
                  ...
                  return (
                    <div key={e.id} className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-2xl border border-slate-100 hover:border-slate-200 hover:bg-slate-50/50 transition">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-400 text-white flex items-center justify-center text-sm font-black flex-shrink-0">
                          {(e.company_name || e.full_name || '?').charAt(0).toUpperCase()}
                        </div>
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <p className="text-sm font-bold text-slate-900 truncate">
                              {e.company_name || e.full_name || 'Unnamed'}
                            </p>
                            {e.verified && (
                              <span title="Verified Employer" className="inline-flex">
                                <CheckCircle2 size={14} className="text-emerald-600 fill-emerald-50" />
                              </span>
                            )}
                          </div>
                        ...
                        <div className="flex gap-4">
                          {[
                            ...
                            <div key={i} className="text-center">
                              <div className="text-base font-black" style={s.color ? { color: s.color } : { color: '#0F172A' }}>{s.val}</div>
